...
void LivoxPointsPlugin::SendRosTf(const ignition::math::Pose3d &pose, const std::string &father_frame,
    ...
    std::string parentName = raySensor->ParentName();
      
      // 去除双冒号前缀，只保留双冒号后面的frame名称
      size_t pos1 = parentName.find("::");
      if (pos1 != std::string::npos)
      {
        parentName = parentName.substr(pos1 + 2);
      }
    tfBroadcaster->sendTransform(
        tf::StampedTransform(tf, ros::Time::now(), parentName, raySensor->Name()));
        std::cout <<raySensor->Name() << std::endl;
}
