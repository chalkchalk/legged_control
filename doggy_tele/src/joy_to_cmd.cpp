...
JoyToCmd::JoyToCmd()
{
    if (!(nh_.param("doggy_joy/max_vel_x", max_vel_x_, 0.1) &&
          ...
          nh_.param("doggy_joy/max_yaw_rate", max_yaw_rate_, 0.5) &&
          nh_.param("doggy_joy/body_height", body_height_, 0.28) &&
...
void JoyToCmd::reset_doggy_cmd()
{
    ...
    doggy_cmd_.cmd_code = doggy_cmd_.GAIT_STANCE;
}
...
void JoyToCmd::joy_msg_callback(const sensor_msgs::Joy &msg)
{
    ...
    if (msg.buttons[BT_LB]) // LB CMD
    {
        ...
        if (msg.buttons[BT_START]) // start
        {
            ...
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_STANCE;
            doggy_cmd_.body_height = 0.0;
        }
    }
    else if (msg.buttons[BT_LT]) // LT GAIT
    {
        if (msg.buttons[BT_A]) // BT_A
        {
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_TYPE1;
        }
        if (msg.buttons[BT_B]) // BT_B
        {
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_TYPE2;
        }
        if (msg.buttons[BT_X]) // BT_X
        {
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_TYPE3;
        }
        if (msg.buttons[BT_Y]) // BT_Y
        {
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_TYPE4;
        }
    ...
    else
    {
        if (msg.buttons[BT_START]) // start
        {
            ...
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_STANCE;
            doggy_cmd_.body_height = body_height_;
        }
        if (msg.buttons[BT_A])
        {
            doggy_cmd_.gait_code = doggy_cmd_.GAIT_TYPE1;
        ...
        if (doggy_cmd_.gait_code != doggy_cmd_.GAIT_STANCE) // moving
        {
        ...
        else
        {
            ...
            doggy_cmd_.yaw_rate = 0;
            if(doggy_cmd_.body_height > 0)
            {
                doggy_cmd_.body_height = body_height_ + msg.axes[3] * body_height_max_delta_;
            }
